...
def recommender_scheduled_job():
    ...
    for user in changed_user:
        try:
            ...
            print(f'{datetime.datetime.now()}  CRONJOB_KDTREE: {user.uid}  STATUS: Completed')
        except FileNotFoundError:
            print(f'{datetime.datetime.now()}  CRONJOB_KDTREE: {user.uid}  STATUS: FileNotFoundError')
        except FirebaseError:
            print(f'{datetime.datetime.now()}  CRONJOB_KDTREE: {user.uid}  STATUS: FirebaseError')
        except InsufficientBookError:
            print(f'{datetime.datetime.now()}  CRONJOB_KDTREE: {user.uid}  STATUS: InsufficientBookError')
